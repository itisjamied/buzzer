...
ws.onmessage = (event) => {
    ...
    switch (data.type) {
        case 'playerUpdate':
            // Update the player list on the host page
            if (isHost) {
                updatePlayerList(data.players);
            }
            break;
        ...
        case 'start':
            // Start a new buzzing round
            switchPage('buzz');

            // Display appropriate elements for host and players
            if (isHost) {
                document.getElementById('buzzStatus').style.display = 'block';
        ...
        case 'buzzUpdate':
            // Update buzz status for the host
            if (isHost) {
                updateBuzzStatus(data.buzzedPlayers, data.totalPlayers);
        ...
        case 'results':
            ...
            // Add "Start New Round" button for the host
            if (isHost) {
                let startNewRoundButton = document.getElementById('startNewRoundButton');
                if (!startNewRoundButton) {
                    startNewRoundButton = document.createElement('button');
